...
//   3. Aguardar autorização do responsável (polling a cada 3s)
...
export function AbastecimentoPublicPage() {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      const { data } = await supabase
        ...
        .eq('id', pendId)
        .single()
      // cast: coluna adicionada em migration 20260921, tipos ainda não regenerados
  ...
  const handlePedido = useFormGuard(async (e: React.FormEvent) => {
    ...
    // Notificar responsável via push
  ...
  const handleFoto = useFormGuard(async (e: React.FormEvent) => {
    ...
    if (gemErr || !gemRes) { setErr('Erro ao processar foto. Tenta novamente.'); setSaving(false); return }
    ...
    // Se confiança baixa ou litros nulos → pedir ao motorista para confirmar manualmente
    if (!litrosLidos || conf === 'baixa') {
      setSaving(false)
      setErr('Não foi possível ler os valores com clareza. Confirme manualmente abaixo.')
      return
    }
  ...
  const handleConfirmarManual = useFormGuard(async (e: React.FormEvent) => {
    e.preventDefault()
    setErr('')
    ...
    setSaving(true)
    const fotoUrl = fotoPreview ? (await supabase.storage.from('combustivel-taloes')
        ...
        <div className="ml-auto flex items-center gap-1">
          {([1, 2, 3, 4] as const).map(n => {
            const ativo = (passo === 'TIPO' && n === 1) || (passo === 'FORM' && n === 2)
                        || (passo === 'AGUARDAR' && n === 3) || ((passo === 'FOTO' || passo === 'DONE') && n === 4)
            const feito = (n === 1 && passo !== 'TIPO') || (n === 2 && !['TIPO', 'FORM'].includes(passo))
            ...
            return (
              <div key={n} className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold transition-all
                ${feito ? 'bg-green-500 text-white' : ativo ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-400'}`}>
                {feito ? '✓' : n}
              </div>
            )
          })}
        ...
        <div className="max-w-md mx-auto">
          ...
          {passo === 'FOTO' && tipo && (
            <form onSubmit={handleFoto} className="space-y-4">
              ...
              <div>
                ...
                {fotoPreview ? (
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => { setFoto(null); setFotoPreview(null); setLitros(null); setCusto(null); if (fotoRef.current) fotoRef.current.value = '' }}
                    ...
                    {litros !== null && (
                      <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-xl text-sm">
                        ...
                        {confianca === 'media' && <p className="text-xs text-amber-600 mt-0.5">Confirma os valores antes de enviar.</p>}
                      </div>
                    )}
                  </div>
                ) : (
              ...
              {/* Preenchimento manual quando Gemini falha */}
              {err && fotoPreview && (
                <form onSubmit={handleConfirmarManual} className="space-y-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              {err && !fotoPreview && <ErrBox msg={err} />}

              {fotoPreview && (!err || litros !== null) && (
            ...
            <div className="text-center py-12 space-y-5">
              ...
              <button
                onClick={() => {
                  ...
                  setLitros(null); setCusto(null); setErr('')
